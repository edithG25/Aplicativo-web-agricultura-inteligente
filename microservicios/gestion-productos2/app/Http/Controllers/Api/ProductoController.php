...
class ProductoController extends Controller
{
    ...
    {
        // Simulación de autorización: Asumimos que el API Gateway o el microservicio de usuarios
        // ya validó el rol y el id_usuario. Aquí solo verificamos que existan.
        // En un escenario real, podrías tener un middleware que haga esto.
        if (!$request->header('X-User-ID') || !$request->header('X-User-Role') || !in_array($request->header('X-User-Role'), ['vendedor', 'administrador'])) {
            return response()->json(['mensaje' => 'Acceso no autorizado. Se requiere rol de vendedor o administrador.'], 403);
        }
        ...
        $validator = Validator::make($request->all(), [
            ...
            // 'id_vendedor' => 'required|integer', // Ya lo obtenemos del header
        ...
        $productoData['id_vendedor'] = $request->header('X-User-ID'); // Asignar el ID del vendedor del header
